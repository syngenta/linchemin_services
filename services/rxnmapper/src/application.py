...
def build_metadata() -> Metadata:
    return Metadata(ci_toolkit=Software(name=TOOLKIT, version=TOOLKIT_VERSION),
...
def process_item(item: Tuple[int, QueryReactionString]):
    ...
    try:
        ...
        output_single = {'query_id': query_id,
        ...
        success = False
        output_single = {'query_id': query_id, 'success': success, 'error_msg': f'problems processing reaction: {e}'}

    return idx, ResultsReactionString(**output_single)


def run_batch(query_data: List[QueryReactionString],
              inp_fmt: str,
              out_fmt: str,
              ) -> RunBatchOut:
    ...
    for (idx, x) in results_raw:
        if x.success:
            ...
            failure_list.append(qdm.get(idx))
    output = {'successes_list': success_list, 'failure_list': failure_list}

    query_parameters = {'inp_fmt': inp_fmt,
                        'out_fmt': out_fmt,
                        }
    outcome = {}

    return RunBatchOut(metadata=build_metadata(),
